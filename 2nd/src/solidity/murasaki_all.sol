...
contract Murasaki_Function_Music_Practice is Ownable, ReentrancyGuard {
    ...
    //internal: get modified delta_sec, +STR/DEX/INT%
    function _get_modified_delta_sec_ofClarinet(uint _summoner, uint _delta_sec) internal view returns (uint) {
        Murasaki_Address ma = Murasaki_Address(address_Murasaki_Address);
        Murasaki_Info mi = Murasaki_Info(ma.address_Murasaki_Info());
        _delta_sec += _delta_sec * mi.strength_withItems(_summoner) / 10000;
        return _delta_sec;
    }
    function _get_modified_delta_sec_ofPiano(uint _summoner, uint _delta_sec) internal view returns (uint) {
        Murasaki_Address ma = Murasaki_Address(address_Murasaki_Address);
        Murasaki_Info mi = Murasaki_Info(ma.address_Murasaki_Info());
        _delta_sec += _delta_sec * mi.dexterity_withItems(_summoner) / 10000;
        return _delta_sec;
    }
    function _get_modified_delta_sec_ofViolin(uint _summoner, uint _delta_sec) internal view returns (uint) {
        Murasaki_Address ma = Murasaki_Address(address_Murasaki_Address);
        Murasaki_Info mi = Murasaki_Info(ma.address_Murasaki_Info());
    ...
    function _calc_level_from_exp(uint _exp) internal pure returns (uint) {
        ...
        if (_exp == 0) {
            _level = 0;
        } else if (_exp < 4464000) {
            _level = 1;
        } else if (_exp < 7300800) {
            _level = 2;
        } else if (_exp < 8812800) {
            _level = 3;
        } else if (_exp < 9964800) {
            _level = 4;
        } else if (_exp < 10972800) {
            _level = 5;
        } else if (_exp < 11908800) {
            _level = 6;
        } else if (_exp < 12801600) {
            _level = 7;
        } else if (_exp < 13680000) {
            _level = 8;
        } else if (_exp < 14572800) {
            _level = 9;
        } else if (_exp < 15494400) {
            _level = 10;
        } else if (_exp < 16473600) {
            _level = 11;
        } else if (_exp < 17539200) {
            _level = 12;
        } else if (_exp < 18734400) {
            _level = 13;
        } else if (_exp < 20102400) {
            _level = 14;
        } else if (_exp < 21744000) {
            _level = 15;
        } else if (_exp < 23846400) {
            _level = 16;
        } else if (_exp < 26769600) {
            _level = 17;
        } else if (_exp < 31622400) {
            _level = 18;
        } else if (_exp < 41472000) {
            _level = 19;
        } else if (_exp >= 41472000) {
            _level = 20;
        }
...
contract BufferVault is Ownable, ReentrancyGuard {
    ...
    function calc_amountNeeded_forTransfer() public view returns (uint, bool) {
        ...
        uint _amountNeeded_inTreasury = _activeSummoners * _amountPerSummoner * (10000 + inflationRate)/100;
        bool _reflex;
