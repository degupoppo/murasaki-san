...
contract Murasaki_Terrarium_Function1 is Ownable, Pausable, ReentrancyGuard {
    ...
    event Set_AllParams(
        ...
        uint _water, 
        uint _light,
        uint[3] _colorOfBall1,
        uint[3] _colorOfBall2,
        uint[3] _colorOfFlame
    );
    function set_allParams (
        ...
        emit Set_AllParams(msg.sender, _nftId, _temp, _water, _light, _colorOfBall1, _colorOfBall2, _colorOfFlame);
    }
    ...
    event Set_Message(address indexed sender, uint _nftId, string _text);
    function set_message (
        ...
        emit Set_Message(msg.sender, _nftId, _text);
    }
    ...
    event Update_Status(address indexed sender, uint _nftId, uint _seltaSec);
    ...
    function _update_status (uint _nftId) internal {
        ...
        emit Update_Status(msg.sender, _nftId, _deltaSec);
    }
    ...
    event Update_GlobalWeather(address indexed sender, uint _deltaSec, uint _weather);
    function _update_globalWeather () internal {
        ...
        if (_deltaSec >= mts.WEATHER_INVERVAL()) {
            ...
            emit Update_GlobalWeather(msg.sender, _deltaSec, _weather);
        }
    }
    ...
    event Fusion(address indexed sender, uint _nftId_fusionTo, uint _nftId_fusionFrom);
    function fusion (uint _nftId_fusionTo, uint _nftId_fusionFrom) external whenNotPaused nonReentrant {
        ...
        emit Fusion(msg.sender, _nftId_fusionTo, _nftId_fusionFrom);
    }
}
...
contract Murasaki_Terrarium_StepupAuction is Ownable, Pausable, ReentrancyGuard {
    ...
    // buy ToM
    event Buy(address indexed sender, uint price);
