...
contract Daycare is Ownable, Pausable, ReentrancyGuard {
    ...
    event Register(uint indexed _summoner, uint _days, uint registered_count);
    function register (uint _summoner, uint _days) external payable nonReentrant whenNotPaused {
        ...
        emit Register(_summoner, _days, registered_count[_summoner]);
    }

    // unregister
    event Unregister(uint indexed _summoner, uint _days, uint registered_count);
    function unregister (uint _summoner, uint _days) external nonReentrant whenNotPaused {
        ...
        emit Unregister(_summoner, _days, registered_count[_summoner]);
    }
    ...
    // exp efficacy = 80%
    event Batch_Caring(address _sender, uint daycare_count, uint daycare_reward);
    function batch_caring (uint[] memory _summoners, uint[] memory _modes) external nonReentrant whenNotPaused {
        ...
        emit Batch_Caring(msg.sender, _count, _count * (PRICE_PER_ACTION - FEE_PER_ACTION));
    }
