...
contract Tiny_Heart is ERC721, Ownable{
    ...
    //global variants
    uint32 public next_heart = 1;
    struct heart {
        uint32 created_item;
        uint32 created_time;
        uint32 created_summoner;
    ...
    function create(
        ...
        require(msg.sender == murasaki_function_address);
        uint32 _now = uint32(block.timestamp);
        uint32 _crafting_heart = next_heart;
    ...
    //TODO: not fixed yet.
    function tokenURI (uint32 _heart) public view returns (string memory) {
        string[9] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
        parts[1] = string(abi.encodePacked("id", " ", toString(_heart)));
        parts[2] = '</text><text x="10" y="40" class="base">';
        parts[3] = string(abi.encodePacked("created summoner", " ", toString(hearts[_heart].created_summoner)));
        parts[4] = '</text><text x="10" y="60" class="base">';
        parts[5] = string(abi.encodePacked("created time", " ", toString(hearts[_heart].created_time)));
        parts[6] = '</text></svg>';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "heart #', toString(_heart), '", "description": "House of Murasaki-san. Murasaki-san is a pet living in your wallet. They grow with your dedication. https://murasaki-san.com/", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));
        return output;
    }
}
...
//---------------------------------------------------------------------------------------------------------------------
//Murasaki Name
//0xd753269537d82D646F59eEdc82498402b29fa6E2
