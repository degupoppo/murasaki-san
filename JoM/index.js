...
//### 1st
    ...
    キャラバン馬車の実装
        作製することで、HoMをたたんで引っ越しが可能となる
        作成後、さらにアドオンやオプションをクラフトすることで能力がupする
        初期地点における中期目標は、移住を可能にするキャラバンを作成することになる
        能力案：
            収納可能資源量up
            移動速度up
        キャラバンで移動中は全く食料・リソースを得られず、移動距離に応じて食料を消費する
    ...
    基本設計概要
        コンセプト
            ...
            目的：
                土地改善による拡大再生産で十分な余剰資源を確保する。
                移住のためのキャラバンを組んで、資源とともに新天地へ旅立つ。
                新天地では効率的な立ち上げが可能であり、ぷち「強くてニューゲーム」。
                    また、新天地でしか得られない資源・建造物を設定する。
        ...
        ワーカープレイスメント
            ...
            ジレンマ：
                すべてのhexにワーカーを配置することはできない
                ワーカーをアクティブにするとお花資源（食料）の消費量が増加する
                食料を得るために配置するのか、リソースを得るために配置するのか。
        リソースマネジメント
            リソースの種類：
                コイン（リソース）、葉っぱ（リソース）、お花（食料資源）
            マネジメント：
                お花の収支コントロール
                    ワーカーを配置してリソースを得るアクションは食料消費を増加させる
                    一方で、gardeningなどの食料を得るアクションはリソースを拡大できない
                    つまり、リソース拡大への投資と食料の貯金は二者択一となる
                コイン、葉っぱの拡大再生産の効率計算
                    リソースを消費して建造物を建てたほうが、
                        将来より多くのリソースを得られるようになる（＝拡大再生産）
                    どの順番でどの建造物を建てると効率が良いかを考える戦略設計が必要
                    また、効率は所有しているhexの性質によっても変化する
                        気候、埋蔵量、特殊建造物、など。
        その他の要素
            リプレイ性：
